...
public class DrinkBlockItem extends BlockItem {
    ...
    public void appendHoverText(ItemStack stack, TooltipContext tooltipContext, List<Component> tooltip, TooltipFlag tooltipFlag) {
        // Get the level from tooltipContext registries if available, otherwise null
        Level world = null;
        if (tooltipContext.registries() != null) {
            // Tooltips are client-side only, so we need to get the level safely
            // We'll use a client-side only helper method
    ...
    // Client-side only helper method
